...
import { AutoresSchema, BooksSchema, EditorialesSchema } from "./schema.ts";
...
const connectMongoDB = async (): Promise<Database> => {
  ...
  if (!mongo_usr || !mongo_pwd || !db_name || !mongo_uri) {
    throw new Error("Missing environment variables, check env.sample for creating .env file",);
  }
...
export const editorialesCollection = db.collection<EditorialesSchema>("Editoriales");
export const autoresCollection = db.collection<AutoresSchema>("Autores");

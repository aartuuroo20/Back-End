...
export const updateSlot = async (context: UpdateSlotContext) => {
    try{
        ...
        const comprobarSlot = await slotsCollection.findOne({day: parseInt(value.day), month: parseInt(value.month), year: parseInt(value.year), hour: parseInt(value.hour), available: true});
        if(!comprobarSlot){
        ...
        const updateSlot = await slotsCollection.updateOne({day: parseInt(value.day), month: parseInt(value.month), year: parseInt(value.year), hour: parseInt(value.hour), available: true}, {$set: {available: false, dni: value.dni}});
